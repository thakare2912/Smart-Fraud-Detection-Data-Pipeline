...
S3_PREFIX = "process/category_trends/"
...
def read_processed_parquet(s3_client):
    ...
    try:
        ...
        required_columns = [
            'category', 'total_sales', 'num_transactions',
        ...
        # Convert data types
        df_all['total_sales'] = df_all['total_sales'].astype('float64')
...
def incremental_load_to_snowflake(conn, df):
    ...
    try:
        ...
        stage_table = "TEMP_STAGE_CATEGORY_TRENDS"
        cursor.execute(f"""
        ...
        WHEN NOT MATCHED THEN INSERT (
            ...
            source.CATEGORY, source.TOTAL_SALES, source.NUM_TRANSACTIONS,
...
def main():
    ...
    logger.info("Starting category trends pipeline...")
